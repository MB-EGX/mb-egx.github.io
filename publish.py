...
# The exact same shard list validate_export_files() requires — these are
# ALWAYS fully regenerated by export_market_matrix() on every run, never
# hand-edited. A rebase conflict touching ONLY files in this set is never
# a real content conflict to resolve line-by-line — whichever copy is
# newest is correct, and that's always the one THIS run just produced
# locally (it was written after the ingest+export steps above, i.e. after
# whatever the remote had). See sync_with_remote_before_push().
_GENERATED_DATA_FILES = {
    "web_public/data/market_data.json",
    "web_public/data/matrix.json",
    "web_public/data/sectors.json",
    "web_public/data/top_10.json",
    "web_public/data/session_picks.json",
    "web_public/data/chart_history.json",
    "web_public/data/ticker_sectors.json",
    "web_public/data/cache_manifest.json",
}


def _rebase_conflicted_files() -> list[str]:
...
def _rebase_in_progress() -> bool:
    head = subprocess.run(["git", "rev-parse", "--git-path", "rebase-merge"], capture_output=True, text=True).stdout.strip()
    head_apply = subprocess.run(["git", "rev-parse", "--git-path", "rebase-apply"], capture_output=True, text=True).stdout.strip()
    return os.path.isdir(head) or os.path.isdir(head_apply)


def _try_auto_resolve_generated_file_conflicts() -> bool:
    """Called only once `git pull --rebase` has already failed with SOME
    conflict. Returns True if every conflicted file across the WHOLE
    rebase was a pure generated-data file and the rebase was carried
    through to completion automatically; False (rebase left aborted) if
    anything else was conflicted, so the caller falls back to the
    original "stop and tell the human" behavior untouched.

    NOTE ON --ours/--theirs DURING A REBASE: this is the opposite of a
    normal `git merge`. A rebase replays OUR commit(s) on top of the
    remote, so git calls the remote side "ours" and our own commit being
    replayed "theirs" — `git checkout --theirs <file>` is what keeps
    OUR freshly-regenerated local copy here, not the remote's. Getting
    this backwards would silently push the OLD/remote data over the
    export this run just produced — worth getting exactly right rather
    than reasoning about it fresh under pressure each time it happens.
    """
    # Bound the loop by a generous cap rather than looping forever if
    # something unexpected keeps the rebase from ever finishing.
    for _ in range(50):
        if not _rebase_in_progress():
            return True  # rebase completed cleanly
        conflicted = _rebase_conflicted_files()
        if not conflicted:
            # Nothing conflicted but the rebase is still "in progress" —
            # shouldn't normally happen; let --continue try to finish it.
            pass
        elif not set(conflicted).issubset(_GENERATED_DATA_FILES):
            non_generated = sorted(set(conflicted) - _GENERATED_DATA_FILES)
            print(f"      Conflict includes non-generated file(s), can't auto-resolve: {non_generated}", file=sys.stderr)
            return False
        else:
            for f in conflicted:
                subprocess.run(["git", "checkout", "--theirs", f], capture_output=True, text=True)
                subprocess.run(["git", "add", f], capture_output=True, text=True)
            print(f"      Auto-resolved generated-data conflict in: {conflicted} (kept this run's freshly-exported copy)")

        cont = subprocess.run(
            ["git", "-c", "core.editor=true", "rebase", "--continue"],
            capture_output=True, text=True,
        )
        if cont.returncode != 0 and not _rebase_in_progress():
            # --continue itself failed for some other reason (not a new
            # conflict) — surface it rather than looping.
            print(cont.stdout, cont.stderr, file=sys.stderr)
            return False
    return False
...
def sync_with_remote_before_push():
    ...
    A rebase can still hit a REAL conflict if the remote's copy of
    market_data.json/ticker_sectors.json/etc. was written by a different
    run than the one this local commit is based on (e.g. two machines/
    runs publishing close together, or a big local data-repair like
    data_repair_tools.py landing between two publish.py runs). Those
    files are pure pipeline OUTPUT — never hand-edited — so there is
    nothing to "merge" between two versions of them; whichever export is
    newest (this run's, always, since it was written after ingest) is
    simply correct. _try_auto_resolve_generated_file_conflicts() handles
    exactly that case automatically. Any conflict touching a file
    OUTSIDE that known generated set (real code, index.html, etc.) still
    aborts and asks a human to look, exactly as before.
    """
    result = subprocess.run(
    ...
    if result.returncode != 0:
        if _try_auto_resolve_generated_file_conflicts():
            print("      Rebase completed after auto-resolving generated-data conflicts.")
            return
        print(result.stdout, result.stderr, file=sys.stderr)
        ...
        raise SystemExit(
            ...
            "with remote changes in a non-generated file (not just the "
            "usual bot commits or generated-data races, which now sync "
            "automatically). The rebase was aborted so your local repo "
            "is back to a clean state. Open a terminal in this folder "
            "and run 'git pull --rebase' yourself to see and resolve "
            "the conflict, then re-run publish.py."
        )

                ...
                try:
                    ...
                    if is_liquid and n_bars >= 20:
                        ...
                        vol_pts = _graduated_score(
                            ...
                            999.0, 999.0, 15.0,  # no meaningful "too much" ceiling for volume build
                        ...
                        if n_bars >= dry_base_n:
                            ...
                            # Baseline = the window BEFORE the recent dry-up window (not a


                            # window that includes it) - otherwise a dry-up is diluted by


                            # its own quiet bars and never measures "recent vs prior".


                            vol_base = df_ind["volume"].iloc[-dry_base_n:-dry_recent_n].mean()
                        ...
                        if n_bars >= 20 and atr and curr_price > 0:
                            ...
                            if atr_series_raw is not None and close_series is not None:
                                ...
                                # Exclude today's own bar from the history: a value is always <= itself,

                                # so including it inflates the contraction percentile by >= 1/n.

                                atr_pct_hist = (atr_series_raw.iloc[-hist_n:-1] / close_series.iloc[-hist_n:-1] * 100.0).dropna()

...
class DatabaseManager:
    ...
    def get_latest_close(self, ticker: str) -> float | None:
        """Latest close for a ticker regardless of whether it appears in
        the current buy_recommendations (owned positions, or names that fell
        below matrix inclusion, are still real prices). Tries both the raw
        symbol and the normalized .CA form since market_data stores whatever
        the feed used. Never raises - returns None on any failure so a
        caller can degrade gracefully (same contract as get_latest_market_date)."""
        candidates = list(dict.fromkeys([str(ticker), self.normalize_symbol(ticker)]))
        with self.get_connection() as conn:
            for c in candidates:
                try:

...
class DatabaseManager:
    ...
    def _init_db(self):
        with self.get_connection() as conn:
            ...
            """)
            # Watchlist enrichment - fundamentals, an external multi-timeframe
            # technical-rating consensus, and provider-computed period returns
            # (see ingestion.py's ENRICHMENT_FIELD_EXACT for exactly which
            # watchlist CSV columns feed this). One row per (ticker, date) -
            # same shape as market_data, since a watchlist export is itself a
            # daily snapshot; history is kept (not overwritten in place) so
            # "what did the P/E look like on date X" stays answerable, not
            # just "what is it today".
            conn.execute("""
                CREATE TABLE IF NOT EXISTS ticker_enrichment (
                    ...
                    dividend DOUBLE,
                    yield_pct DOUBLE,
                    return_daily_pct DOUBLE,
                    return_1w_pct DOUBLE,
                    return_1m_pct DOUBLE,
                    return_ytd_pct DOUBLE,
                    return_1y_pct DOUBLE,
                    return_3y_pct DOUBLE,
                    rating_5min VARCHAR,
                    rating_15min VARCHAR,
                    rating_30min VARCHAR,
                    rating_hourly VARCHAR,
                    rating_5hour VARCHAR,
                    rating_daily VARCHAR,
                    rating_weekly VARCHAR,
                    rating_monthly VARCHAR,
                    rating_consensus_score DOUBLE,
                    PRIMARY KEY (ticker, date)
                );
            """)
            conn.execute("""
    ...
    def get_latest_enrichment(self) -> dict:
        """{normalized_ticker: {field: value, ...}} for every ticker's
        MOST RECENT ticker_enrichment row (see that table's docstring in
        _init_db and ingestion.py's ENRICHMENT_FIELD_EXACT for what feeds
        it) - fundamentals, the 8-timeframe external rating consensus
        score, and provider-computed period returns. A ticker never fed
        via an enrichment-bearing watchlist CSV simply has no key here;
        callers (decision_matrix.py) must treat a missing ticker the same
        as "not available", never as zero/neutral - matching every other
        optional-feature contract in this app (e.g. sector_rotation.py's
        "available: False").
        """
        with self.get_connection() as conn:
            try:
                rows = conn.cursor().execute("""
                    SELECT ticker, market_cap, revenue, avg_vol_3m, eps, pe_ratio, beta,
                           dividend, yield_pct, return_daily_pct, return_1w_pct, return_1m_pct,
                           return_ytd_pct, return_1y_pct, return_3y_pct, rating_consensus_score
                    FROM (
                        SELECT *, ROW_NUMBER() OVER (PARTITION BY ticker ORDER BY date DESC) AS rn
                        FROM ticker_enrichment
                    ) WHERE rn = 1;
                """).fetchall()
            except Exception as e:
                logger.error(f"get_latest_enrichment() failed: {e}")
                return {}

        cols = ["market_cap", "revenue", "avg_vol_3m", "eps", "pe_ratio", "beta", "dividend",
                "yield_pct", "return_daily_pct", "return_1w_pct", "return_1m_pct",
                "return_ytd_pct", "return_1y_pct", "return_3y_pct", "rating_consensus_score"]
        out = {}
        for r in rows:
            ticker = str(r[0]).strip().upper()
            values = dict(zip(cols, r[1:]))
            out[ticker] = out[self.normalize_symbol(ticker)] = values
        return out
